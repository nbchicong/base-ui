...
/**
 * @class ColumnFileUpload
 * @extends GridEditor
 * @param options
 * @constructor
 */
var ColumnFileUpload = function (options) {
  CT.apply(this, options || {});
  this.isChange = false;
  ColumnFileUpload.superclass.constructor.call(this);
};
CT.extend(ColumnFileUpload, GridEditor, {
  render: function (v) {
    if (!v) {
  ...
  edit: function (v) {
    ...
    var __onChange = __column.onChange || function () {};
    ...
    this.getEl().on('click', '[data-action="remove"]', function () {
      var $inputFileUpload = _this.getEl().find('#input-file-upload');
...
ColumnManager.registerType('fileUpload', ColumnFileUpload);
